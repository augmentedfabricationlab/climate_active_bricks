...
def assembly_interfaces_numpy(assembly,
    ...
    blocks = [assembly.element(key) for key in assembly.network.vertices()]
    nmax = min(nmax, len(blocks))
    block_cloud = assembly.network.get_vertices_attributes('xyz')
    ...
    for k in assembly.network.vertices():
        ...
        if face_face:
            ...
            for f0, (origin, uvw) in frames.items():
                ...
                for j in nbrs:
                    ...
                    xyz = array(nbr._mesh.get_vertices_attributes('xyz'), dtype=float64).reshape((-1, 3)).T

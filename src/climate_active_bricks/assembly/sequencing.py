...
def assembly_building_sequence(assembly, key):
    ...
    course = assembly.network.get_vertex_attribute(key, 'course')
    ...
    while tovisit:
        ...
        if k not in seen:
            seen.add(k)
            course = assembly.network.get_vertex_attribute(k, 'course')

            if course_above == course + 1:
                sequence.append(k)
                for nbr in assembly.network.vertex_neighbors(k):
                    if nbr not in seen:
